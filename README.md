...
- 🔍 **Search** songs, albums, artists and playlists on YouTube Music —
  and while the box is empty it doubles as Browse (like Spotify mobile):
  personal **Discover** picks and public **Charts & Moods**
...
- 💚 **Spotify playlist import** — menu → *Import from Spotify…*: paste any
  ...
  needed; re-import any time to pull updates. Optionally add your own free
  Spotify API credentials in Settings to import very long playlists
  completely (Riff auto-falls back to the keyless method when the API
  refuses, e.g. for Spotify's own editorial playlists)
...
- 🎛️ **Now Playing panel** — Spotify-style right sidebar (`Alt+Shift+N` or
...
- ✨ **Discover** (inside Search) — privacy-preserving recommendations:
...
| Discover new music | Sidebar → *Search* (empty box) → *Discover* (personal) or *Charts & Moods* (public) |
